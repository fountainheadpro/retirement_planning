...
def test_erp_haircut_scales_excess_over_tbills():
    stock = np.array([0.12, 0.02])
    tbill = np.array([0.02, 0.02])
    np.testing.assert_allclose(apply_erp_haircut(stock, tbill, 0.5), [0.07, 0.02])
...
def test_followup_experiment_runner_returns_labeled_rows():
    ...
    assert "4% TIPS-floor proxy 50%" in labels
